    ...
    public partial class Form1 : Form
    {
            ...
            if (pc.TargetID != pc.ID)
            {
                do
                {
                    ...
                    foreach (string item in attacks)
                    {
                        ...
                        try
                        {
                            if (Convert.ToDouble(parseitem[1]) > .1)
                            {
        ...
        private void combat()
        {
            ...
            while ((tar.IsDead == true || tar.Health == 0) && tar.Name != "") //ADD INVENTORY FULL CHECK
            {
                ...
                PauseForMilliSeconds(300);
                if (lootcounter >= 6) { lootcounter = 0; keyenumerator("ESC"); break; }
                killLabel.Text = "Kills: " + kills;
            }
            lootcounter = 0;
        ...
        public void changewaypoint()
        {
            ...
            if (pc.Distance3D((float)awaypoint.X, (float)awaypoint.Y, (float)awaypoint.Z) < 3) //hit waypoint CHANGE THIS
            {
