...
namespace AngelBot
{
    public partial class Form1 : Form
    {
        ...
        public void loadsettings()
        {
            ...
            healdelay = Int32.Parse(ini.IniReadValue("limits", "HealDelay"));
            tmrheal.Interval = healdelay * 1000;
            ...
            if (pc.TargetID != pc.ID)
            {
                do
                {
                    ...
                    foreach (string item in attacks)
                    {
                        ...
                        if (targetstar.ID != pc.ID && targetstar.Type == eType.Player) //if mob doesnt have me targeted
                        {
        ...
        private void combat()
        {
            ...
            while ((tar.IsDead == true || tar.Health == 0) && tar.Name != "") //ADD INVENTORY FULL CHECK
            {
                ...
                PauseForMilliSeconds(300);
                if (lootcounter >= 8) { lootcounter = 0; keyenumerator("ESC"); break; }
                killLabel.Text = "Kills: " + kills;
            }
            lootcounter = 0;
            ...
            if (btnstop.Visible == true && tar.TargetID != pc.ID)
            {
                ...
                findmob();
                //keybd_event((int)Keys.W, (byte)MapVirtualKey((int)Keys.W, 0), 2, 0);//stop running for sure
            }//SEARCH FOR MOB
        ...
        private void findmob()
        {
            ...
            if (isresting == false)
            {
                while (tar.Type != eType.AttackableNPC || (((int)tar.Level <= ignorelevel) || hasplayer == true || ignorelist.Contains(tar.Name)))
                {
                    ...
                    waymovement();
                    if (tmrstuck.Enabled == false) tmrstuck.Start();
        ...
        public void changewaypoint()
        {
            ...
            if (pc.Distance3D((float)awaypoint.X, (float)awaypoint.Y, (float)awaypoint.Z) < 2.5) //hit waypoint CHANGE THIS
            {
            ...
            lblwprange.Text = "WPRange: " + string.Format("{0:f2}", pc.Distance3D((float)awaypoint.X, (float)awaypoint.Y, (float)awaypoint.Z));
        }
        public cwaypoint findclosestwaypoint()
        {
            ...
            foreach (cwaypoint currentpoint in waypointlist){
                ...
                double distwp = pc.Distance3D((float)currentpoint.X, (float)currentpoint.Y, (float)currentpoint.Z);
                double newwp = pc.Distance3D((float)closestpoint.X, (float)closestpoint.Y, (float)closestpoint.Z);
        ...
        {
            double distance = pc.Distance3D((float)awaypoint.X, (float)awaypoint.Y, (float)awaypoint.Z);
            ...
            if (stuckcounter >= 3) 
            { 
                keyenumerator("SPACE"); PauseForMilliSeconds(400); keyenumerator("SPACE");
                returncounter++;
                stuckcounter = 0;
            }
            if (returncounter >= 3) {
