    ...
    public partial class Form1 : Form
    {
        ...
        public void loadsettings()
        {
            ...
            try
            {
                ...
                shutofftime = Int32.Parse(ini["limits"]["Shutoff"].TrimEnd(' '));
                if (shutofftime > 0) tmrshutoff.Interval = shutofftime * 60000;
                deathresttime = Int32.Parse(ini["limits"]["DeathRest"].TrimEnd(' '));
                ...
                keystrafeL = Convert.ToString(ini["keybinds"]["StrafeL"]);
                keystrafeR = Convert.ToString(ini["keybinds"]["StrafeR"]);
            ...
            if (ishealer == true && healready == true && pc.IsCasting == false)
            {
                if (((Convert.ToDouble(pc.Health) / Convert.ToDouble(pc.MaxHealth)) * 100) < healhp)
                {
                    ...
                    while (prevhealth >= pc.Health)
                    {
                        ...
                        PauseForMilliSeconds(50);
                        if (pc.Health == 0) return;
                        if (pc.IsCasting == true) PauseForMilliSeconds(Convert.ToInt32(healdelay * 1000));
                        if (prevhealth > pc.Health) prevhealth = pc.Health;
                    }
                    healready = false;
                    //Application.DoEvents;
        ...
        public bool adddetect()
        {
            ...
            elist.Update();
            lblstatus.Text = "Status: Checking for ADD";
        ...
        public void resurrect()
        {
            ...
            for (int y = -190; y < 91; y = y + 20)
            {
                for (int x = 111; x > -190; x = x - 20)
                {
        ...
        private void buffcast()
        {
            ...
            for (int i = 0; i <= 4; i++)
            {
                if (buffsready[i] == true)
                {
                    ...
                    else if (i == 1) tmrbuff2.Start();
                    else if (i == 2) tmrbuff3.Start();
                    else if (i == 3) tmrbuff4.Start();
                    else if (i == 4) tmrbuff5.Start();
        ...
        {
            lootcounter = 0;
            while ((tar.IsDead == true || tar.Health == 0) && tar.Name != "" && (pc.CubeFull() == false)) //ADD INVENTORY FULL CHECK
            {
        ...
        private void combat()
        {
            ...
            if (attackflag == false)
            {
                while (incombatloop == true)
                {
                    ...
                    if ((tar.IsDead != true && tar.Name != "") && tar.TargetID == pc.ID && tar.Type == eType.AttackableNPC) //ADD DETECTION/FIGHT
                    {
                        int tarlevel1;
        ...
        private void findmob()
        {
            ...
            if (isresting == false)
            {
                ...
                while (tar.Type != eType.AttackableNPC || (((int)tar.Level <= ignorelevel) || hasplayer == true || ignorelist.Contains(tar.Name) || ignorelist.Contains(tar.ID.ToString())))
                {
                    ...
                    if (pc.Name == tar.Name) { PauseForMilliSeconds(100); keyenumerator("ESC"); }
                    Application.DoEvents();
        ...
        public void changewaypoint()
        {
            if (deathrun == true)
            {
                ...
                else { awaypoint = deathpointlist[0]; deathcounter = 0; };
                if (pc.Distance3D(awaypoint.X, awaypoint.Y, awaypoint.Z) < 2) //hit waypoint CHANGE THIS
                {
                    deathcounter++;
                }
                if (deathcounter >= deathpointlist.Count) deathcounter = deathpointlist.Count - 1;
            }
            else
            {
                ...
                if (movecounter < 0) { movecounter = 0; forward = true; }
                if (movecounter >= waypointlist.Count) { movecounter = waypointlist.Count - 1; forward = false; }
        ...
        public cwaypoint findclosestwaypoint()
        {
            ...
            try
            {
                ...
                foreach (cwaypoint currentpoint in waypointlist)
                {
                    ...
                    double distwp = pc.Distance3D(currentpoint.X, currentpoint.Y, currentpoint.Z);
                    double newwp = pc.Distance3D(closestpoint.X, closestpoint.Y, closestpoint.Z);
                    if (distwp < newwp) { closestpoint = currentpoint; deathrun = false; }
                }
                //ADD CHECKS FOR DEATH POINT SYSTEM IF CLOSER
                if (pc.Distance3D(closestpoint.X, closestpoint.Y, closestpoint.Z) > 80)
                {
                    foreach (cwaypoint currentpoint in deathpointlist)
                    {
                        ...
                        double distwp = pc.Distance3D(currentpoint.X, currentpoint.Y, currentpoint.Z);
                        double newwp = pc.Distance3D(closestpoint.X, closestpoint.Y, closestpoint.Z);
                        if (distwp < newwp) { closestpoint = currentpoint; deathcounter = dcounter; deathrun = true; }
                        dcounter++;
                    }
                }
            }
            ...
            */
            ...
            if (awaypoint.Face(pc.Y, pc.X) > 0)
            {
                if ((pc.Rotation > (awaypoint.Face(pc.Y, pc.X) - 180)) && (pc.Rotation < (awaypoint.Face(pc.Y, pc.X)) - 5)) //left within 1.5 degrees
                {
                    ...
                    //tmrstuck.Stop();
                    while ((pc.Rotation >= (awaypoint.Face(pc.Y, pc.X) - 180)) && (pc.Rotation <= (awaypoint.Face(pc.Y, pc.X)) - 5))  //LEFT
                    {
                        //if (tar.Health < 100 || tar.HasTarget == true) hasplayer = true; else hasplayer = false;
                ...
                else if (((-180 < (awaypoint.Face(pc.Y, pc.X) - 180)) && pc.Rotation < (awaypoint.Face(pc.Y, pc.X) - 180)) || (pc.Rotation > (awaypoint.Face(pc.Y, pc.X)) + 5)) //right within 1.5 degrees
                {
                    ...
                    //tmrstuck.Stop();
                    while (((-180 < (awaypoint.Face(pc.Y, pc.X) - 180)) && pc.Rotation < (awaypoint.Face(pc.Y, pc.X) - 180)) || (pc.Rotation > (awaypoint.Face(pc.Y, pc.X)) + 5)) //RIGHT
                    {
                        //if (tar.Health < 100 || tar.HasTarget == true) hasplayer = true; else hasplayer = false;
                        //if (tar.Type != eType.AttackableNPC || (((int)tar.Level <= ignorelevel) || hasplayer == true || ignorelist.Contains(tar.Name))) return;
                        //changewaypoint();
            ...
            else //NEG POINT
            {
                if ((pc.Rotation > awaypoint.Face(pc.Y, pc.X) + 5) && (pc.Rotation < (awaypoint.Face(pc.Y, pc.X)) + 180)) //left within 1.5 degrees
                {
                    keybd_event((int)Keys.Right, (byte)MapVirtualKey((int)Keys.Right, 0), 0, 0); //  Down
                    //tmrstuck.Stop();
                    while ((pc.Rotation > awaypoint.Face(pc.Y, pc.X) + 5) && (pc.Rotation < (awaypoint.Face(pc.Y, pc.X)) + 180)) //RIGHT
                    {
                        //if (tar.Health < 100 || tar.HasTarget == true) hasplayer = true; else hasplayer = false;
                        //if (tar.Type != eType.AttackableNPC || (((int)tar.Level <= ignorelevel) || hasplayer == true || ignorelist.Contains(tar.Name))) return;
                        //changewaypoint();
                ...
                else if (((-180 < awaypoint.Face(pc.Y, pc.X)) && pc.Rotation < awaypoint.Face(pc.Y, pc.X) - 5) || (pc.Rotation > (awaypoint.Face(pc.Y, pc.X) + 180))) //right within 1.5 degrees
                {
                    keybd_event((int)Keys.Left, (byte)MapVirtualKey((int)Keys.Left, 0), 0, 0); //  Down
                    //tmrstuck.Stop();
                    while (((-180 < awaypoint.Face(pc.Y, pc.X)) && pc.Rotation < awaypoint.Face(pc.Y, pc.X) - 5) || (pc.Rotation > (awaypoint.Face(pc.Y, pc.X) + 180)))//LEFT
                    {
                        //if (tar.Health < 100 || tar.HasTarget == true) hasplayer = true; else hasplayer = false;
                        //if (tar.Type != eType.AttackableNPC || (((int)tar.Level <= ignorelevel) || hasplayer == true || ignorelist.Contains(tar.Name))) return;
                        //changewaypoint();
                    ...
                    if (distance < 0.4)
                    {
                        if (stuckcounter == 3 || stuckcounter == 4)
                        {
                            if (pc.IsCasting == false)
                            {
                                ...
                                returncounter++;
                                if (returncounter == 1) keyenumerator(keystrafeL);
                                if (returncounter == 2) keyenumerator(keystrafeR);
                            }//stuckcounter = 0;
                        }
                    }
