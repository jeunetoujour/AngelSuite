...
namespace AionMemory
{
    ...
    public class EntityList : IEnumerable<Entity>
    {
        ...
        void TraverseNode(int nodePtr)
        {
            try
            {
                int entityPtr = Memory.ReadInt(Process.handle, nodePtr + 12);

                if (entityPtr == 0 || entityPtr == stop || !found.Add(entityPtr))
                    return;

                int leftPtr = Memory.ReadInt(Process.handle, nodePtr);
                int rightPtr = Memory.ReadInt(Process.handle, nodePtr + 4);
