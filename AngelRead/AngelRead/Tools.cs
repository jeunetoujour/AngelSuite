...
namespace AngelRead
{
    public static class Tools
    {
        ...
        public static ILocation2D RotateAroundOrigin2D(float x, float y, double radians)
        {
            float dx = x * (float)Math.Cos(radians) + y * (float)Math.Sin(radians);
            float dy = y * (float)Math.Cos(radians) - x * (float)Math.Sin(radians);
        ...
        public static ILocation2D RotateAroundPoint2D(float x, float y, double radians, float xOrigin, float yOrigin)
        {
            x -= xOrigin;
            y -= yOrigin;

            var newLocation = RotateAroundOrigin2D(x, y, radians);

            x = newLocation.X + xOrigin;
            y = newLocation.Y + yOrigin;
