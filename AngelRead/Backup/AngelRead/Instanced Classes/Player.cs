...
namespace AngelRead
{
    ...
    public class Player : Entity
    {
        ...
        const uint PLAYER_Y_OFFSET = 0x85AC; //neg
        const uint PLAYER_Z_OFFSET = 0x85A8; //neg
        const uint PLAYER_ROT_OFFSET = 0xAE48; //neg
        const uint PLAYER_KINAHPTR_OFFSET = 0xB5D4; //neg
        const uint PLAYER_SPELL_OFFSET = 0xA240; //neg
        ...
        public int GetUsedSpace()
        {
            ...
            for (int slot = 0; slot < MaxSlots; slot++)
            {
                ...
                //int thisItemCount = (int)Memory.ReadUInt(Process.handle, (uint)(thisItem + 0xA0)); //WORKS
                //string thisItemName = Memory.ReadString(Process.handle, (int)Memory.ReadUInt(Process.handle, (uint)(thisItem + 0x1C)),64,true);
        ...
        public void Updatenamelvl()
        {
            ...
            this.ID = (int)Memory.ReadUInt(Process.handle, (uint)(Process.Modules.Game + PLAYER_INFOADDRESS_OFFSET - PLAYER_GUID_OFFSET));//A1CC58//0xA1CC5C
            getpcptr();
