...
public static class AutoUpdate
{
    ...
    public static UpdateStatus UpdateFiles(string RemotePath, bool bWantDebug)
    {
        ...
        if (bWantDebug) 
            Log = "... our assembly name is: " + AssemblyName;
        if (bWantDebug) 
            Log = "... our remote URI is:    " + RemoteUri + UpdateFileName;
        ...
        try
        {
            ...
            if (Info.Length > 1) { 
                if (bWantDebug) Log = "... the server's latest version is:              " + GetVersion(Info[1]);
            }
            ...
            if (Application.StartupPath.ToLower() + "\\" + Info[0].ToLower() == Application.ExecutablePath.ToLower())
            {
                ...
                foreach (string F in FileList)
                {
                    ...
                    if (isToUpgrade)
                    {
                        if (MessageBox.Show("Update " + Info[0] + " to latest?", "Yes/No", MessageBoxButtons.YesNo) == DialogResult.Yes)
                        {
