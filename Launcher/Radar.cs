...
namespace AngelSuite
{
    public partial class Radar : Form
    {
        ...
        //private AionMemory.Party party = null;
        int heartbeat = 0;
        ...
        private void tmrRadarUpdate_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                if (Process.handle == IntPtr.Zero)
                {
                    ...
                    aionOffsets.Update();
                    //player.PLAYER_INFOADDRESS_OFFSET = aionOffsets.playerInfoAddress;
                    //player.ENTITY_OFFSET = aionOffsets.entityInfoAddress;
                    //player.PLAYER_GUID_OFFSET = aionOffsets.pGUIDInfoAddress;
                    //elist.ENTITYLIST_OFFSET = aionOffsets.entityInfoAddress;
                    
                }
                if (Process.handle == IntPtr.Zero)
                    this.tmrRadarUpdate.Interval = 500;
                else
                {
                    this.tmrRadarUpdate.Interval = 125;
                    ...
                    foreach (var entity in elist)
                    {
                        try
                        {
                            ...
                            if (sType == "Collectable")
                                sType = "Gatherable";
                            else if (sType == "GatherableNoSkill")
                                sType = "Gatherable";
