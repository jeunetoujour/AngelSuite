    ...
    public partial class Form1 : Form
    {
        ...
        public void loadsettings()
        {
            ...
            try
            {
                ...
                /*string healtemp = GetSetting(ini, initemplate, "heals", "Heals");
                if (healtemp != "")
                {
                    if (healtemp.StartsWith("|")) healtemp = healtemp.Substring(1, healtemp.Length);
                    if (attacktemp.Contains('\0').ToString() == "True")
                    {
                        attacktemp = attacktemp.Substring(0, attacktemp.LastIndexOf('\0') - 0);
                    }
                    string[] listheal = healtemp.Split('|');
                    heals.AddRange(listheal);
                }*/
        ...
        private void UpdateReactions()
        {
            ...
            RControlList = AddControl(Memory.ReadInt(Process.handle, (uint)((Process.Modules.Game + 0xA687C0))), -1, RControlList); //A657D0  
        ...
        private void attackmob()
        {
            ...
            if (tar.Type == eType.AttackableNPC && (tar.Health == 100 || targetstar.Type == eType.FriendlyNPC))//can I fight it? && tar.HasTarget != true
            {
                ...
                if (Attacklist.Nodes[0].Nodes.Count != 0)
                {
                    ...
                    while (tar.Stance != eStance.Dead && pc.Health != 0 && btnstop.Visible == true && tar.TargetID != pc.ID) //Top level IS THIS LOOP REQUIRED?
                    {
                        string skillname = Attacklist.SelectedNode.Text;
                        string[] info = Convert.ToString(Attacklist.SelectedNode.Tag).Split('|');
                ...
                if (ents.Type == eType.AttackableNPC && ents.Name != "")
                {
                    if (ents.TargetID == pc.ID && ents.Health > 0)
                    {
                        ...
                        while (tar.ID != ents.ID)
                        {
                            ...
                            if (trycount >= 5) { keyenumerator("ESC"); return true; }
                            PauseForMilliSeconds(400);
                            tar.UpdateID();
                            if (savelog == true) tw.WriteLine(DateTime.Now + " Trying to target ADD: " + ents.Name + " MobTarget: " + tar.Name);
                            trycount++;
                        }
                        //mainattackloop(0);
        ...
        private void rest()
        {
            ...
            try
            {
                ...
                if (pc.Health < pc.MaxHealth) healcast();
        ...
        public void sellmisc()
        {
            ...
            nControlList = AddControl(Memory.ReadInt(Process.handle, (Process.Modules.Game + 0xA686A4)), -1, nControlList); //Resurrection
        ...
        public void resurrect()
        {
            ...
            nControlList = AddControl(Memory.ReadInt(Process.handle, (Process.Modules.Game + 0xA6877C)), -1, nControlList); //Resurrection
        ...
        private void combat()
        {
            ...
            if (attackflag == false)
            {
                while (incombatloop == true)
                {
                    ...
                    if (pc.ID == tar.ID && keypressing == false) { PauseForMilliSeconds(100); keyenumerator("ESC"); }
